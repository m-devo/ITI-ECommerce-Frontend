...
export class CartService {
  ...
  constructor() {
    this.authService.isLoggedIn$
      .pipe(startWith(null as boolean | null), pairwise())
  ...
  private updateCartState(response: ApiResponse<Cart>): void {
    if (!response || !response.success) {
      console.error(
        '%c API request failed or response was falsy',
    ...
    if (cart) {
      ...
      console.warn(
        '%c Request is success but data type is wrong [CartService]',
        'color: orange;',
        response
      );
  ...
  private normalizeCart(rawCart: any): Cart {
    ...
    const normalizedItems: CartItem[] = rawCart.items
      .map((item: any) => {
        if (item.book) {
          return item;
        }
  ...
  private handleError(operation: string) {
    return (error: any): Observable<null> => {
      console.error(
        `%c CartService failed [${operation}]`,
        'color: red; font-weight: bold;',
        error
      );
  ...
  getCart(): Observable<ApiResponse<Cart> | null> {
    ...
    return this.http.get<ApiResponse<Cart>>(this.URL).pipe(
      tap((response) => {
        if (response?.success) {
          console.log(
            '%c Data has been retrieved successfully',
  ...
  updateCart(
    cartItems: CartUpdateRequest[]
  ): Observable<ApiResponse<Cart> | null> {
    this._cartIsLoading.next(true);
    return this.http
      .put<ApiResponse<Cart>>(`${this.URL}/update`, cartItems)
      .pipe(
  ...
  addItem(book: Book, quantityToAdd: number = 1): void {
    ...
    const existingItem = currentCart?.items.find(
      (item: any) => item.book?._id === book._id || item.book === book._id
    );
  ...
  incrementItem(bookId: string): Observable<ApiResponse<Cart> | null> {
    this._cartIsLoading.next(true);
    return this.http
      .put<ApiResponse<Cart>>(`${this.URL}/items/${bookId}/increment`, {})
      .pipe(
  ...
  decrementItem(bookId: string): Observable<ApiResponse<Cart> | null> {
    this._cartIsLoading.next(true);
    return this.http
      .put<ApiResponse<Cart>>(`${this.URL}/items/${bookId}/decrement`, {})
      .pipe(
        tap(this.updateCartState.bind(this)),
        finalize(() => this._cartIsLoading.next(false)),
  ...
  removeItem(bookId: string): Observable<ApiResponse<Cart> | null> {
    this._cartIsLoading.next(true);
    return this.http
      .delete<ApiResponse<Cart>>(`${this.URL}/items/${bookId}`)
      .pipe(
        tap((res) => {
  ...
  addToCart(bookId: string, quantity: number) {
    ...
    this.http
      .put<ApiResponse>(`${environment.apiUrl}/cart/update`, data)
      .subscribe({
        next: (response) => {
          if (response.success) {
            this.cartItems = response.data.items;
